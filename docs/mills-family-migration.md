...
Record the legacy files and lines retired in the rollout report. Count only code made unreachable after the rollback observation period; shared adapters and retained Mills domain/security code are not removal. Pin an immutable package version and lockfile integrity, build the exported package before rollout, and preserve the previous artifact for rollback.
