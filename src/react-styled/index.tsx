...
import { useConversationLauncherBinding, type ConversationWorkspaceReadable } from "../react/workspace.js";
    ...
    <main className="hr-chat__body">
      <Transcript className="hr-chat__transcript" {...(renderToolResult ? { renderToolResult } : {})}
        {...(props.messageActions === false ? {} : { renderMessage: (message, _index, context) => <div>
          <Message message={message} error={context.error} toolCalls={context.toolCalls} {...(renderToolResult ? { renderToolResult } : {})}/>
...
function BoundHandrailChat<TRequest>(props: HandrailChatProps<TRequest>): ReactNode {
  ...
  void _runtime; void _plugins; void _composer; void _presence;
...
export function StyledChatLauncher(props: StyledChatLauncherProps): ReactNode {
  const binding = useConversationLauncherBinding(props.workspace, props.connectionStatus);
  const { workspace: _workspace, connectionStatus: _connectionStatus, trigger, ...preset } = props;
  ...
  return <ChatLauncherRoot {...binding}><ChatLauncherTrigger className="hr-chat__launcher-trigger">
    {trigger ?? "Open chat"}<ChatLauncherBadge/><ChatLauncherStatus className="hr-chat__sr"/>
  </ChatLauncherTrigger>
